-- Current Storage preflight includes mimetype/contentLength (HTTP body, not file size).
-- Only compare size when Storage supplies the finalized file-size field.
begin;
create or replace function chat_private.storage_allowed(path text,meta jsonb,operation text) returns boolean language sql stable security definer set search_path='' as $$
 ...
 when 'insert' then a.uploader_id=auth.uid() and a.message_id is null
...
create policy avatar_upload on storage.objects for insert to authenticated with check(bucket_id='profile-avatars' and name ~ ('^'||auth.uid()::text||'/[0-9a-f-]{36}$')
 and (metadata->>'size' is null or (metadata->>'size')::bigint between 1 and 2097152)
 and (metadata->>'mimetype' is null or metadata->>'mimetype' in ('image/png','image/jpeg','image/webp','image/gif')));
commit;
